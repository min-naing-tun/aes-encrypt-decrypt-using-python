...
for i in range(4):
	userInput.append(input("Enter hex ROW "+str(i+1)+" : ").split())

# string = input("Enter hex to sBox : ").split()

result = ""
for i in range(4):
	for j in range(4):
		if(j==3):
			result += sbox[userInput[i][j]]
			result += "\n"
		else:
			result += sbox[userInput[i][j]]+" "
...
# 	if((i+1)%4==0):
# 		result += sbox[string[i]]+"\n"
# 	else:
# 		result += sbox[string[i]]+" "
print("after inverse sub byte")
print(result)
...
# # for i in range(4):
# # 	userInput1.append(input("Enter List (0 0 0 0) : ").split())
